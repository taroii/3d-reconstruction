...
def main():
    ...
    ap.add_argument("--n", type=int, default=60, help="held-out val frames to score")
    ...
    import random
    frames = DS.build_frames([args.dataset], "val")
    random.seed(0); random.shuffle(frames); frames = frames[:args.n]

    # streaming accumulators (global + high-|K| tail)
    ...
    print(f"\nframes scored {len(frames)}  valid px {int(G['n'])}")
    ...
    print(f"\nverdict: head captures curvature IFF TOP-tail corr is clearly >0 AND "
          f"TOP-tail MAE model < predict-zero.")
